...
class TestKalmanFilter(PbTestCase):
    ...
    def test_invalid_bayes(self):
        k = pb.KalmanFilter(**self.setup_2)
        self.assertRaises(TypeError, k.bayes, np.array([1.]), np.array([3., 2.]))

    def test_copy(self):
        """Test that copying KalmanFilter works as expected"""
    ...
    def test_deepcopy(self):
        """Test that deep copying KalmanFilter works as expected"""
        o = pb.KalmanFilter(**self.setup_2)  # original
        c = deepcopy(o)  # copy
        self.assertEqual(type(c), type(o))

        self.assertTrue(id(o) != id(c))
        for (a, b) in [(o.A, c.A), (o.C, c.C), (o.Q, c.Q), (o.R, c.R)]:
...
class TestControlKalmanFilter(PbTestCase):
    """Tests for kalman filter that takes control vector in bayes()"""
    ...
    def test_init(self):
        k = pb.ControlKalmanFilter(**self.setup_1)
        self.assertEqual(type(k), pb.ControlKalmanFilter)
        l = pb.ControlKalmanFilter(**self.setup_2)
        self.assertEqual(type(l), pb.ControlKalmanFilter)
    ...
    def test_invalid_init(self):
        ...
        for arg in args:
            ...
            setup[arg] = 125.65
            self.assertRaises(TypeError, pb.ControlKalmanFilter, **setup)
        ...
        for arg in args:
            ...
            self.assertRaises(ValueError, pb.ControlKalmanFilter, **setup)
    ...
    def test_bayes(self):
        k = pb.ControlKalmanFilter(**self.setup_2)
        y = np.array([[4.1], [-0.2], [1.4], [-2.1]])
    ...
    def test_copy(self):
        """Test that copying ControlKalmanFilter works as expected"""
        o = pb.ControlKalmanFilter(**self.setup_1)  # original
        c = copy(o)  # copy
        self.assertEqual(type(c), type(o))
    ...
    def test_deepcopy(self):
        """Test that deep copying ControlKalmanFilter works as expected"""
        o = pb.ControlKalmanFilter(**self.setup_2)  # original
        c = deepcopy(o)  # copy
        self.assertEqual(type(c), type(o))
