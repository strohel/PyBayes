...
\begin{description}
	\item[call-by-value convention] \hfill \\
		ensures that caller function does not change variables passed as parameters from caller
		scope by copying them at function call time. This provides clear semantics but incurs
	...
	\item[call-by-reference convention] \hfill \\
		...
		to caller function. This approach minimises function call overhead but may appear confusing
	...
	\item[call-by-object (call-by-sharing) convention] \hfill \\
		...
		but cannot be used to assign to referred objects. When an object is marked as immutable,
...
to implement, they impose unnecessary restrictions on library design and cause overhead in places
where it could be avoided.

Last discussed aspect of programming languages relates to memory management:
\begin{description}
	\item[garbage-collected] \hfill \\
		provide memory management for... TODO
	\item[non garbage-collected] \hfill \\
		put the burden of memory management on the shoulders of programmer: she is responsible for
		correctly reclaiming resources when they are no longer in use.
\end{description}
