...
Our intended audience is broad scientific community interested in the field of recursive Bayesian
estimation and decision-making. Keeping this in mind and in order to formalise expectations for
desired library for Bayesian estimation, following set of requirements was developed.
...
mindset. Combined with the fact that syntax of mentioned functionally-oriented languages differs
...
compensating this with more thorough test-suite.
...
\begin{description}
	\item[compiled implementations] \hfill \\
		...
		advantages are zero overhead, no requirement to install additional software (interpreter)
		on target machine.
...
conventions is used for Bayesian estimation library; while call-by-value-only languages can be simpler
to implement, we are convinced that they impose unnecessary restrictions on library design and cause
overhead in places where it could be avoided.
...
like a library for Bayesian estimation targeting wide adoption. We also believe that automatic
...
When used as an implementation language for desired library for recursive Bayesian estimation, we
...
\begin{description}
	...
	\item[low overhead] \hfill \\
		...
		Computer Language Benchmarks Game\footnote{http://shootout.alioth.debian.org/}), C++ is
		hard to outperform by a significant margin (
\end{description}
set of drawbacks
