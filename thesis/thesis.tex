...
depends on system state in \(t-1\) (and is not affected by previous states)},
...
\begin{align}
	...
	\forall i \in \hat{N}: \omega_i \geq 0 \quad \sum_{i=1}^N \omega_i = 1 \notag
\end{align}
...
expected likelihood of the system being in state in small neighbourhood of \(x_t^{(i)}\);
...
\begin{enumerate}
	\item for each \(i \in \hat{N}\) compute \(x_t^{(i)}\) by random sampling from conditional {\pdf}
	...
	\item for each \(i \in \hat{N}\) compute weight \(\omega_t^{(i)}\) using \eqref{eq:PFWeightUpdate}
...
Relative computational ease that can be seen in above algorithm description comes with
...
proved that particle filter converges to true a~posteriori density with
\(N \rightarrow \infty\), therefore the number of particles should be chosen as a~balance of accuracy and
speed.\cite{Smi:10}
...
\begin{enumerate} \addtocounter{enumi}{3} % so that enumeration starts with number 4
	\item for each \(i \in \hat{N}\) resample \(x_t^{(i)}\) from approximate a posteriori {\pdf}
...
Step~4 therefore facilitates avoidance of low-weight particles by replacing them with more weighted
ones.
